# AI Guardrails Developer Guide

Prompt sanitization, tool risk checks, PII redaction, and output moderation.
...
- You need **guardrails**, **validation**, **policy enforcement** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/ai-guardrails` instead of reaching into app-specific internals.
...
| `framework/libraries/ai-guardrails` | Nested publishable library package. |
| `framework/libraries/ai-guardrails/src` | Public runtime source and exported modules. |
| `framework/libraries/ai-guardrails/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `defineGuardrailPolicy`, `sanitizePrompt`, `evaluateToolRisk`, `moderateOutput`, `assertGuardrails` |
...
- Hosts should import from `@platform/ai-guardrails`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/ai-guardrails"]
...
```ts
import { packageId, packageDisplayName, defineGuardrailPolicy } from "@platform/ai-guardrails";

console.log(packageId, packageDisplayName);
console.log(typeof defineGuardrailPolicy);
```
...
- Publishes 0 public modules from `@platform/ai-guardrails`.
- Exports 14 named symbols through the public entrypoint, including `packageId`, `packageDisplayName`, `packageDescription`, `defineGuardrailPolicy`, `sanitizePrompt`, `evaluateToolRisk`, and more.
