...
| Primary focus | guardrails, validation, policy enforcement |
...
| Named Exports | 14 | `packageId`, `packageDisplayName`, `packageDescription`, `defineGuardrailPolicy`, `sanitizePrompt`, `evaluateToolRisk`, `moderateOutput`, `assertGuardrails` |
...
console.log(packageId, packageDisplayName, typeof defineGuardrailPolicy);
```

Use the root repo scripts for day-to-day work after the workspace is bootstrapped, or run the nested package directly from `framework/libraries/ai-guardrails` if you need lower-level control.
